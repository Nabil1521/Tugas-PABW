...
class CARSEARCH(Resource):
    def get(self, searchkey):
        rows = TBCarsWeb.select().where(
            (TBCarsWeb.carname.contains(searchkey)) | 
            (TBCarsWeb.carbrand.contains(searchkey)) |
            (TBCarsWeb.carmodel.contains(searchkey))
        )
        datas=[]
        for row in rows:
            datas.append({
...
api.add_resource(CARSEARCH, '/search/<searchkey>', endpoint="search/")
